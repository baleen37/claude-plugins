...
function runBunInstall() {
  return new Promise((resolve, reject) => {
    ...
    console.error('[databricks-devtools] Installing dependencies (first run only)...');
    ...
    child.on('exit', (code) => {
      if (code === 0) {
        console.error('[databricks-devtools] Dependencies installed successfully.');
        resolve();
      } else {
        console.error('[databricks-devtools] ERROR: Failed to install dependencies.');
    ...
    child.on('error', (err) => {
      console.error(`[databricks-devtools] ERROR: Failed to run bun install: ${err.message}`);
      reject(err);
    });
  });
}
...
function runBuild() {
  return new Promise((resolve, reject) => {
    ...
    console.error('[databricks-devtools] Building plugin...');
    ...
    child.on('exit', (code) => {
      if (code === 0) {
        console.error('[databricks-devtools] Build completed successfully.');
        resolve();
      } else {
        console.error('[databricks-devtools] ERROR: Build failed.');
    ...
    child.on('error', (err) => {
      console.error(`[databricks-devtools] ERROR: Failed to run build: ${err.message}`);
      reject(err);
    });
  });
}
...
async function main() {
  try {
    ...
    if (!existsSync(mcpServerPath)) {
      console.error(`[databricks-devtools] ERROR: MCP server not found at ${mcpServerPath}`);
    ...
    child.on('error', (err) => {
      console.error(`[databricks-devtools] ERROR: Failed to start MCP server: ${err.message}`);
      process.exit(1);
    });

  } catch (error) {
    console.error(`[databricks-devtools] ERROR: ${error.message}`);
    process.exit(1);
  }
}

main().catch((error) => {
  console.error(`[databricks-devtools] Unexpected error: ${error.message}`);
  process.exit(1);
});
