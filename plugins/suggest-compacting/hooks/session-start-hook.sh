#!/bin/bash
# Suggest Compacting SessionStart Hook
...
# Store session_id in ENV_FILE for use by suggest-compacting hook
ENV_FILE="${CLAUDE_ENV_FILE:-}"
if [[ -z "$ENV_FILE" ]]; then
    # Create state directory if it doesn't exist
    mkdir -p ~/.claude/suggest-compacting
    ENV_FILE="$HOME/.claude/suggest-compacting/session-env.sh"
fi
...
if [[ -w "$ENV_FILE" ]]; then
    ...
    echo "export SUGGEST_COMPACTING_SESSION_ID=$SESSION_ID" >> "$ENV_FILE"
fi
