...
if [ "$BEHIND" -gt 0 ]; then
    echo "⚠️  Branch is $BEHIND commits behind origin/$BASE" >&2
    echo "   Consider: git merge origin/$BASE" >&2
else
...
# Quick conflict check (using improved detection)
...
if [ -n "$MERGE_BASE" ]; then
    ...
    if [ "$HAS_CONFLICT" = true ]; then
        ...
        echo "   Run conflict-check.sh for details" >&2
    else
        echo "✓ No merge conflicts detected"
    fi
else
    echo "⚠️  Could not check conflicts (no merge base)" >&2
fi
echo
...
if [ "$DIFF_LINES" -gt 250 ]; then
    echo "⚠️  WARNING: PR is $DIFF_LINES lines (recommended: <250)" >&2
    echo "   Consider breaking into smaller PRs for faster review" >&2
fi
echo
